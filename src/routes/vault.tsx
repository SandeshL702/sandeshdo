...
import { hashPin, pinMatches } from "@/lib/crypto";
  ...
  if (!unlocked) {
    return (
      <main className="overflow-x-hidden px-5 pt-5 pb-8">
        ...
        <p className="mt-2 text-sm text-muted">{pinHash ? t("lock.title") : t("vault.setPin")}</p>
        <p className="mt-1 text-xs text-muted">{t("vault.pinHint")}</p>
        <form
          className="mt-6"
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          ...
          />
          {err && <p className="mt-2 text-xs text-overdue">{t("vault.wrong")}</p>}
          <Button type="submit" className="mt-4 w-full" disabled={pin.length < 4}>
            {pinHash ? t("vault.unlock") : t("vault.setPin")}
          </Button>
        </form>
      </main>
    );
    ...
    <main className="overflow-x-hidden px-5 pt-5 pb-8">
      <div className="flex items-start justify-between gap-3">
        <div>
          <BrandMark compact />
          <h1 className="font-display mt-3 text-title leading-none font-medium tracking-tight">{t("vault.title")}</h1>
          <p className="mt-1.5 text-sm text-muted">{t("vault.empty")}</p>
        </div>
        <HeaderActions />
      </div>
      <button
